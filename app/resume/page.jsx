...
const Resume = () => {
  return (
    <div
      className="min-h-[110vh] flex items-center justify-center py-12 xl:py-10 "
    >
      <div className="container mx-auto">
        ...
        >
          ...
          <motion.div variants={fadeIn("left", 0.3)}
            ...
            animate="show" className="grid grid-cols-1 lg:grid-col-2 gap-[30px] ">
                    {experience.items.map((item, index) => {
                      return <li key={index} className="bg-white h-[184px] py-6 px-10 rounded-xl flex flex-col justify-center items-center lg:items-start gap-1 ">
                        ...
                        <h3 className="text-xl max-w-[260px] min-w-[60px] text-primaryText text-center lg:text-left ">{item.position}</h3>
            ...
            <div className="flex flex-col gap-[30px] text-center xl:text-left ">
                <h3 className="text-4xl font-bold text-primaryText">{education.title}</h3>
                <p className="max-w-[600px] text-primaryText/60 mx-auto xl:mx-0  ">{education.description}</p>
                <ScrollArea className="h-[400px]  ">
                  <ul className="grid grid-cols-1 lg:grid-col-2 gap-[30px] ">
                    {education.items.map((item, index) => {
                      return <li key={index} className="bg-white h-[184px] py-6 px-10 rounded-xl flex flex-col justify-center items-center lg:items-start gap-1 ">
                        <span className="text-accent-Default">{item.duration}</span>
                        <h3 className="text-xl max-w-[260px] min-w-[60px] text-primaryText text-center lg:text-left ">{item.degree}</h3>
                        <div className="flex items-center gap-3">
                          <span className="w-[6px] h-[6px] rounded-full bg-accent-Default "></span>
            ...
            <div className="flex flex-col gap-[30px]">
                <div className="flex flex-col gap-[30px] text-center xl:text-left ">
                <h3 className="text-4xl font-bold text-primaryText">{skills.title}</h3>
                <p className="max-w-[600px] text-primaryText/60 mx-auto xl:mx-0  ">{skills.description}</p>
                </div>
                <ScrollArea className="h-[400px]  ">
                        ...
                        <TooltipProvider delayDuration={100}>
                          <Tooltip>
                            <TooltipTrigger className="w-full relative h-[150px] bg-white rounded-xl flex justify-center items-center group ">
                              <div className="text-6xl  text-primaryText z-10 rounded-[10px] ">{item.icon}</div>
                              <div className="w-full h-full transition-all rounded-[10px]  ease-in-out absolute top-0 left-0 z-9 duration-500 group-hover:bg-accent-gold"></div>
                            </TooltipTrigger>
                            <TooltipContent>
            ...
            <TabsContent value="about" className="w-full text-center xl:text-left">
              <div className="flex flex-col gap-[30px]  ">
                ...
                <p className="max-w-[600px] text-primaryText/60 mx-auto xl:mx-0 ">{about.description}</p>
                <ul className="grid mx-auto xl:mx-0 grid-cols-1 xl:grid-cols-2 gap-y-6 max-w-[628px] ">
                  {about.info.map((item, index) => {
                    return <li key={index} className="flex items-center justify-center gap-4 xl:justify-start">
                     <span className="text-primaryText">{item.fieldName}</span>
                     <span className="text-xl text-primaryText/50">{item.fieldValue}</span>
                    </li>
                  })}
                </ul>
              </div>
